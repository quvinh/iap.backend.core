...
namespace App\Services\FormulaCategorySold;
...
class FormulaCategorySoldService extends \App\Services\BaseService implements IFormulaCategorySoldService
{
    private ?IFormulaCategorySoldRepository $formulaCategorySoldRepos = null;

    public function __construct(IFormulaCategorySoldRepository $repos)
    {
        $this->formulaCategorySoldRepos = $repos;
    }
     ...
     */
    public function getSingleObject(int $id, array $withs = []): FormulaCategorySold
    {
        try {
            $query = $this->formulaCategorySoldRepos->queryOnAField(['id', $id]);
            $query = $this->formulaCategorySoldRepos->with($withs, $query);
            $record = $query->first();
     ...
     * @param array<string> $withs
     * @return Collection<int,FormulaCategorySold>
    ...
    {
        try {
            $query = $this->formulaCategorySoldRepos->search();
            if (isset($rawConditions['name'])) {
                $param = StringHelper::escapeLikeQueryParameter($rawConditions['name']);
                $query = $this->formulaCategorySoldRepos->queryOnAField([DB::raw("upper(name)"), 'LIKE BINARY', DB::raw("upper(concat('%', ? , '%'))")], positionalBindings: ['name' => $param]);
            }

            if (isset($rawConditions['updated_date'])) {
                $query = $this->formulaCategorySoldRepos->queryOnDateRangeField($query, 'updated_at', $rawConditions['updated_date']);
            }
            if (isset($rawConditions['created_date'])) {
                $query = $this->formulaCategorySoldRepos->queryOnDateRangeField($query, 'created_at', $rawConditions['created_date']);
            }
            ...
            if (isset($rawConditions['sort'])) {
                ...
                return $this->formulaCategorySoldRepos->sort($query, $sort)->get();
            }
            return $query->get();
        } catch (Exception $e) {
     ...
     */
    public function create(array $param, MetaInfo $commandMetaInfo = null): FormulaCategorySold
    {
        DB::beginTransaction();
        try {
            #1 Create
            $record = $this->formulaCategorySoldRepos->create($param, $commandMetaInfo);
            DB::commit();
            #2 Return
            return $record;
        } catch (\Exception $e) {
     ...
     */
    public function update(int $id, array $param, MetaInfo $commandMetaInfo = null): FormulaCategorySold
    {
        DB::beginTransaction();
        try {
            #1: Can edit? -> Yes: move to #2 No: return Exception with error
            $record = $this->formulaCategorySoldRepos->getSingleObject($id)->first();
            if (empty($record)) {
                throw new RecordIsNotFoundException();
            }
            #2: update
            ...
            $record = $this->formulaCategorySoldRepos->update($param, $commandMetaInfo);
    ...
    public function delete(int $id, bool $softDelete = true, MetaInfo $commandMetaInfo = null): bool
    {
        ...
        try {
            ...
            $result =  $this->formulaCategorySoldRepos->delete(id: $id, soft: $softDelete, meta: $commandMetaInfo);
            DB::commit();
            return $result;
        } catch (\Exception $ex) {
            DB::rollBack();
