...
class TemplateService extends \App\Services\BaseService implements ITemplateService
{
    private ?ITemplateRepository $templateRepos = null;

    public function __construct(ITemplateRepository $repos)
    {
        $this->templateRepos = $repos;
    }
     ...
     */
    public function getSingleObject(int $id, array $withs = []): Template
    {
        try {
            $query = $this->templateRepos->queryOnAField(['id', $id]);
            $query = $this->templateRepos->with($withs, $query);
            $record = $query->first();
    ...
    {
        try {
            $query = $this->templateRepos->search();
            if (isset($rawConditions['name'])) {
                $param = StringHelper::escapeLikeQueryParameter($rawConditions['name']);
                $query = $this->templateRepos->queryOnAField([DB::raw("upper(name)"), 'LIKE BINARY', DB::raw("upper(concat('%', ? , '%'))")], positionalBindings: ['name' => $param]);
            }

            if (isset($rawConditions['updated_date'])) {
                $query = $this->templateRepos->queryOnDateRangeField($query, 'updated_at', $rawConditions['updated_date']);
            }
            if (isset($rawConditions['created_date'])) {
                $query = $this->templateRepos->queryOnDateRangeField($query, 'created_at', $rawConditions['created_date']);
            }
            ...
            if (isset($rawConditions['sort'])) {
                ...
                return $this->templateRepos->sort($query, $sort)->get();
            }
            return $query->get();
        } catch (Exception $e) {
     ...
     */
    public function create(array $param, MetaInfo $commandMetaInfo = null): Template
    {
        DB::beginTransaction();
        try {
            #1 Create
            $record = $this->templateRepos->create($param, $commandMetaInfo);
            DB::commit();
            #2 Return
            return $record;
        } catch (\Exception $e) {
     ...
     */
    public function update(int $id, array $param, MetaInfo $commandMetaInfo = null): Template
    {
        DB::beginTransaction();
        try {
            #1: Can edit? -> Yes: move to #2 No: return Exception with error
            $record = $this->templateRepos->getSingleObject($id)->first();
            if (empty($record)) {
                throw new RecordIsNotFoundException();
            }
            #2: update
            ...
            $record = $this->templateRepos->update($param, $commandMetaInfo);
    ...
    public function delete(int $id, bool $softDelete = true, MetaInfo $commandMetaInfo = null): bool
    {
        ...
        try {
            ...
            $result =  $this->templateRepos->delete(id: $id, soft: $softDelete, meta: $commandMetaInfo);
            DB::commit();
            return $result;
        } catch (\Exception $ex) {
            DB::rollBack();
