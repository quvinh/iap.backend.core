...
class TaxFreeVoucherService extends \App\Services\BaseService implements ITaxFreeVoucherService
{
    private ?ITaxFreeVoucherRepository $taxFreeVoucherRepos = null;

    public function __construct(ITaxFreeVoucherRepository $repos)
    {
        $this->taxFreeVoucherRepos = $repos;
    }
     ...
     * @return TaxFreeVoucher
     * @throws RecordIsNotFoundException
     */
    public function getSingleObject(int $id, array $withs = []): TaxFreeVoucher
    {
        try {
            $query = $this->taxFreeVoucherRepos->queryOnAField(['id', $id]);
            $query = $this->taxFreeVoucherRepos->with($withs, $query);
            $record = $query->first();
     ...
     * @param array<string> $withs
     * @return Collection<int,TaxFreeVoucher>
    ...
    {
        try {
            $query = $this->taxFreeVoucherRepos->search();
            if (isset($rawConditions['name'])) {
                $param = StringHelper::escapeLikeQueryParameter($rawConditions['name']);
                $query = $this->taxFreeVoucherRepos->queryOnAField([DB::raw("upper(name)"), 'LIKE BINARY', DB::raw("upper(concat('%', ? , '%'))")], positionalBindings: ['name' => $param]);
            }

            if (isset($rawConditions['updated_date'])) {
                $query = $this->taxFreeVoucherRepos->queryOnDateRangeField($query, 'updated_at', $rawConditions['updated_date']);
            }
            if (isset($rawConditions['created_date'])) {
                $query = $this->taxFreeVoucherRepos->queryOnDateRangeField($query, 'created_at', $rawConditions['created_date']);
            }
            ...
            if (isset($rawConditions['sort'])) {
                ...
                return $this->taxFreeVoucherRepos->sort($query, $sort)->get();
            }
            return $query->get();
        } catch (Exception $e) {
     ...
     */
    public function create(array $param, MetaInfo $commandMetaInfo = null): TaxFreeVoucher
    {
        DB::beginTransaction();
        try {
            #1 Create
            $record = $this->taxFreeVoucherRepos->create($param, $commandMetaInfo);
            DB::commit();
            #2 Return
            return $record;
        } catch (\Exception $e) {
     ...
     */
    public function update(int $id, array $param, MetaInfo $commandMetaInfo = null): TaxFreeVoucher
    {
        DB::beginTransaction();
        try {
            #1: Can edit? -> Yes: move to #2 No: return Exception with error
            $record = $this->taxFreeVoucherRepos->getSingleObject($id)->first();
            if (empty($record)) {
                throw new RecordIsNotFoundException();
            }
            #2: update
            ...
            $record = $this->taxFreeVoucherRepos->update($param, $commandMetaInfo);
